...
if (!empty($tasks)) {
    ...
    $assignee_query = "SELECT ta.task_id, ta.status as assignee_status, ta.started_at, ta.completed_at,
                      ...
                      WHERE ta.task_id IN ($task_ids_string) AND ta.deleted_at IS NULL
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['create_task'])) {
    ...
    if (empty($errors)) {
        ...
        try {
            // Insert task record
            $task_insert = "INSERT INTO tasks (name, description, priority, creator_id, due_date, organization_id) 
                          VALUES (?, ?, ?, ?, ?, ?)";
            $stmt = $conn->prepare($task_insert);
            $stmt->bind_param('sssisi', $name, $description, $priority, $user_id, $due_date, $organization_id);
            $stmt->execute();
                ...
                <tbody>
                    <?php foreach ($tasks as $task): ?>
                        <tr class="task-row" 
                            ...
                            <td>
                                <?php $status_class = 'status-badge ' . $task['status']; ?>
                                <span class="<?php echo $status_class; ?>">
                                    <?php 
                                        $status_display = str_replace('_', ' ', $task['status']);
