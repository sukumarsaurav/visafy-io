...
$query = "SELECT asv.*, 
         (SELECT COUNT(DISTINCT ad.id) FROM application_documents ad WHERE ad.application_id = asv.application_id) as document_count,
         (SELECT COUNT(DISTINCT ac.id) FROM application_comments ac WHERE ac.application_id = asv.application_id) as comment_count,
         (SELECT COUNT(DISTINCT ad.id) FROM application_documents ad WHERE ad.application_id = asv.application_id) as total_documents,
         (SELECT COUNT(DISTINCT ad.id) FROM application_documents ad WHERE ad.application_id = asv.application_id AND ad.status = 'approved') as approved_documents,
         (SELECT COUNT(DISTINCT ad.id) FROM application_documents ad WHERE ad.application_id = asv.application_id AND ad.status = 'pending') as pending_documents,
         (SELECT COUNT(DISTINCT ad.id) FROM application_documents ad WHERE ad.application_id = asv.application_id AND ad.status IN ('submitted', 'approved')) as received_documents,
         (SELECT aa.team_member_id FROM application_assignments aa WHERE aa.application_id = asv.application_id AND aa.status = 'active' ORDER BY aa.assigned_at DESC LIMIT 1) as team_member_id,
         (SELECT CONCAT(u.first_name, ' ', u.last_name) FROM application_assignments aa 
          JOIN team_members tm ON aa.team_member_id = tm.id 
          JOIN users u ON tm.member_user_id = u.id 
          WHERE aa.application_id = asv.application_id AND aa.status = 'active' 
          ORDER BY aa.assigned_at DESC LIMIT 1) as case_manager_name,
         (SELECT tm.member_type FROM application_assignments aa 
          JOIN team_members tm ON aa.team_member_id = tm.id 
          WHERE aa.application_id = asv.application_id AND aa.status = 'active' 
          ORDER BY aa.assigned_at DESC LIMIT 1) as case_manager_role,
         aps.color as status_color,
         u.email as applicant_email,
         a.priority,
         a.submission_status
         FROM application_services_view asv 
         JOIN application_statuses aps ON asv.status_id = aps.id
         JOIN users u ON asv.applicant_id = u.id
         JOIN applications a ON asv.application_id = a.id
         ORDER BY 
         CASE 
...
$team_query = "SELECT tm.id, tm.member_type as role, u.first_name, u.last_name 
               FROM team_members tm 
               JOIN users u ON tm.member_user_id = u.id 
               WHERE u.status = 'active' AND tm.invitation_status = 'accepted'";
$team_members = [];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['create_application'])) {
    ...
    $organization_id = $_SESSION['organization_id']; // Get from session
    $consultant_id = $user_id; // Current logged in user is the consultant
    ...
    try {
        ...
        $insert_query = "INSERT INTO applications (user_id, visa_id, booking_id, status_id, expected_completion_date, notes, priority, created_by, organization_id, consultant_id) 
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($insert_query);
        $stmt->bind_param('iiissssiii', $user_id, $visa_id, $booking_id, $status_id, $expected_completion_date, $notes, $priority, $user_id, $organization_id, $consultant_id);
        $stmt->execute();
        ...
        $docs_query = "INSERT INTO application_documents (application_id, document_type_id, status, organization_id)
                      SELECT ?, vrd.document_type_id, 'pending', ?
                      FROM visa_required_documents vrd
                      WHERE vrd.visa_id = ?";
        $stmt = $conn->prepare($docs_query);
        $stmt->bind_param('iii', $application_id, $organization_id, $visa_id);
        $stmt->execute();
        $stmt->close();
        ...
        <div class="filter-section">
            ...
            <select id="visa-filter" class="filter-select">
                ...
                <?php foreach ($visas as $visa): ?>
                    <option value="<?php echo $visa['visa_id']; ?>">
                        <?php echo $visa['visa_type'] . ' (' . $visa['country_name'] . ')'; ?>
                    </option>
                <?php endforeach; ?>
            </select>
        </div>
            ...
            <table class="applications-table" id="applications-table">
                ...
                <tbody>
                    <?php foreach ($applications as $app): ?>
                        <tr data-status="<?php echo isset($app['status_id']) ? $app['status_id'] : '0'; ?>"
                            data-visa="<?php echo isset($app['visa_id']) ? $app['visa_id'] : ''; ?>"
                            data-priority="<?php echo isset($app['priority']) ? $app['priority'] : 'normal'; ?>">
                            <td>
                                <a href="view_application.php?id=<?php echo $app['application_id']; ?>" class="reference-link">
                            ...
                            <td>
                                <div class="applicant-info">
                                    <span
                                        class="applicant-name"><?php echo isset($app['applicant_name']) ? htmlspecialchars($app['applicant_name']) : 'Unknown'; ?></span>
                                    <span
                                        class="applicant-email"><?php echo isset($app['applicant_email']) ? htmlspecialchars($app['applicant_email']) : ''; ?></span>
                                </div>
                            </td>
                            <td>
                                <?php
                            ...
                            <td>
                                <span class="status-badge"
                                    style="background-color: <?php echo isset($app['status_color']) ? $app['status_color'] : '#808080'; ?>10; color: <?php echo isset($app['status_color']) ? $app['status_color'] : '#808080'; ?>;">
                                    <i class="fas fa-circle"></i>
                                    <?php echo ucwords(str_replace('_', ' ', isset($app['status_name']) ? $app['status_name'] : 'Unknown')); ?>
                                </span>
                            </td>
                            <td>
                                <?php if (isset($app['case_manager_name']) && !empty($app['case_manager_name'])): ?>
                                    <div class="case-manager">
                                        ...
                                        <span
                                            class="manager-role"><?php echo isset($app['case_manager_role']) ? htmlspecialchars($app['case_manager_role']) : ''; ?></span>
                                    </div>
                                <?php else: ?>
                            ...
                            <td>
                                <div class="documents-info">
                                    <div class="status-breakdown">
                                        <span class="status-item status-asked" title="Asked (Total Required)">
                                            <i class="fas fa-file-alt"></i> <?php echo isset($app['total_documents']) ? $app['total_documents'] : 0; ?>
                                        </span>
                                        <span class="status-item status-pending" title="Pending">
                                            <i class="fas fa-clock"></i> <?php echo isset($app['pending_documents']) ? $app['pending_documents'] : 0; ?>
                                        </span>
                                        <span class="status-item status-received" title="Received">
                                            <i class="fas fa-check-circle"></i> <?php echo isset($app['received_documents']) ? $app['received_documents'] : 0; ?>
                                        </span>
                                    </div>
                                </div>
                            </td>
                            <td>
                                <?php 
                                $status_name = isset($app['status_name']) ? strtolower($app['status_name']) : '';
                                $is_completed = ($status_name === 'completed' || $status_name === 'approved');
                                ?>
                                <span class="app-status-badge <?php echo $is_completed ? 'app-completed' : 'app-inprogress'; ?>">
                                    <?php echo $is_completed ? 'Completed' : 'In Progress'; ?>
                                </span>
                            </td>
                            <td>
                                <?php 
                                $submission = isset($app['submission_status']) ? $app['submission_status'] : 'draft';
                                $submission_labels = [
                                    'draft' => ['label' => 'Draft', 'class' => 'sub-draft'],
                                    'reviewed' => ['label' => 'Reviewed', 'class' => 'sub-reviewed'],
                                    'client_signoff' => ['label' => 'Client Sign-off', 'class' => 'sub-signoff'],
                                    'submitted' => ['label' => 'Submitted', 'class' => 'sub-submitted']
                                ];
                                $sub_info = isset($submission_labels[$submission]) ? $submission_labels[$submission] : $submission_labels['draft'];
                                ?>
                                <span class="submission-badge <?php echo $sub_info['class']; ?>">
                                    <?php echo $sub_info['label']; ?>
                                </span>
                            </td>
                            <td>
                                <span
                                    class="priority-badge priority-<?php echo isset($app['priority']) ? $app['priority'] : 'normal'; ?>">
                                    <?php echo ucfirst(isset($app['priority']) ? $app['priority'] : 'normal'); ?>
                                </span>
                            </td>
                            <td>
                                <span
                                    class="date"><?php echo isset($app['created_at']) ? date('M d, Y', strtotime($app['created_at'])) : 'Unknown'; ?></span>
                            </td>
                            <td class="actions-cell">
                                ...
                                <button type="button" class="btn-action btn-status" title="Update Status"
                                    onclick="openStatusModal(<?php echo $app['application_id']; ?>, <?php echo isset($app['status_id']) ? $app['status_id'] : '0'; ?>)">
                                    <i class="fas fa-exchange-alt"></i>
                                </button>

                                <button type="button" class="btn-action btn-assign" title="Assign Case Manager"
                                    onclick="openAssignModal(<?php echo $app['application_id']; ?>, '<?php echo isset($app['team_member_id']) ? $app['team_member_id'] : ''; ?>')">
                                    <i class="fas fa-user-plus"></i>
                                </button>

                                <a href="application_documents.php?id=<?php echo $app['application_id']; ?>"
                                    class="btn-action btn-document" title="Manage Documents">
                                    <i class="fas fa-file-alt"></i>
                                </a>

                                <a href="application_comments.php?id=<?php echo $app['application_id']; ?>"
                                    class="btn-action btn-comment"
                                    title="Comments (<?php echo isset($app['comment_count']) ? $app['comment_count'] : '0'; ?>)">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="applications.php" method="POST" id="assignForm">
                    ...
                    <div class="form-buttons">
                        <button type="button" class="btn cancel-btn" data-dismiss="modal">Cancel</button>
                        <button type="submit" name="assign_application" class="btn submit-btn">Assign Case
                            Manager</button>
                    </div>
                </form>
            </div>
            ...
            <div class="modal-body">
                <form action="applications.php" method="POST" id="createApplicationForm">
                        ...
                        <div class="form-group">
                            ...
                            <select name="booking_id" id="booking_id" class="form-control">
                                ...
                                $bookings_query = "SELECT b.id, b.reference_number, CONCAT(u.first_name, ' ', u.last_name) AS client_name 
                                                 ...
                                                 AND bs.name IN ('confirmed', 'completed')
                                                 AND b.deleted_at IS NULL
                                                 ORDER BY b.booking_datetime DESC
                                                 LIMIT 50";
                                $bookings_stmt = $conn->prepare($bookings_query);
                                $org_id = $_SESSION['organization_id'];
                                $bookings_stmt->bind_param('i', $org_id);
                    ...
                    <div class="step-content" id="step-5">
                        ...
                        <div class="review-section">
                            <div id="review-summary">
                                ...
                                <div class="review-item">
                                    <strong>Booking:</strong> <span id="review-booking">None</span>
                                </div>
                                <div class="review-item">
                    ...
                    <div class="form-navigation">
                        ...
                        <button type="submit" name="create_application" class="btn submit-btn" id="submitApplicationBtn"
                            style="display:none;">
                            Create Application
                        </button>
                    </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function updateReviewInfo() {
            ...
            document.getElementById('review-booking').textContent = bookingSelect.options[bookingSelect.selectedIndex]?.text || 'None';
